 ...
 * primitive set. Adapted from the DeepSeek Harness's own test helper
 * (packages/storage/storage-domain/tests/helpers/memory-backend.ts,
 * deepseek-harness, MIT) to run against the published
 * `@deepseek-ai/dsh-storage` contract — the identical pattern the harness's
 * domain suite uses. It lives under `tests/`, never `src/`, so it stays out
 * of the published surface.
 *
 * Fidelity to the backend contract: version stamping and `version-mismatch`
 * on reopen, `malformed` never (memory cannot corrupt), per-call atomicity,
 * `closed` after close, delete idempotence. Media survive across backends
 * through the shared `media` map passed into the constructor, which
 * simulates process restarts; stamp `versions` directly to fabricate an
 * on-medium version and force a `version-mismatch` without a prior open.
 */
...
export class MemoryMediaPool {
   ...
   * per rejection. Negative path: callers assert their state is untouched
